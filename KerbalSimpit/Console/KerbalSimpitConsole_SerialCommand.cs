...
namespace KerbalSimpit.Console
{
    class KerbalSimpitConsole_SerialCommand : KerbalSimpitConsole.SimpitConsoleCommand
    {
       // Command strings
        ...
        private static readonly string SERIAL_STATUS_COMMAND = Localizer.GetStringByTag(Command_Lib.serial_command_tag(Command_Lib.SIM_SERIAL_COMMAND_STATUS));
        private static readonly string SERIAL_START_COMMAND = Localizer.GetStringByTag(Command_Lib.serial_command_tag(Command_Lib.SIM_SERIAL_COMMAND_START));
        private static readonly string SERIAL_STOP_COMMAND = Localizer.GetStringByTag(Command_Lib.serial_command_tag(Command_Lib.SIM_SERIAL_COMMAND_STOP));


        // Port Status
        private static readonly string SERIAL_PORT_CONNECTED = Localizer.GetStringByTag(Command_Lib.serial_misc_tag(Command_Lib.SIM_SERIAL_MISC_CONNECTED_CAPS));
        ...
        public override void Simpit_Command_Call(KerbalSimpitConsole.Command_Arguments command_args)
        {
            ...
            if(command_args.arguments[0] == SERIAL_STATUS_COMMAND)
            {
                print_serial_status();
            }

            // If the command is a serial start command
            if(command_args.arguments[0] == SERIAL_START_COMMAND)
            {
                Debug.Log("Serial start called");
                // If the serial port has already been connected to, run this
                if (KSPit.run_connect)
                {
                    if (KSPit.serial_ports.First().Value.port_connected == false)
                    {
                        KSPit.init_ports(k_simpit);
                    }
                }
            ...
            if(command_args.arguments[0] == SERIAL_STOP_COMMAND)
            {
                if(KSPit.serial_ports.First().Value.port_connected == true)
                {
                    KSPit.kill_ports(k_simpit);
                }
            }
        }
        ...
        private void print_serial_status()
        {
            ...
            foreach (KeyValuePair<string, KSPit.port_data> entry in KSPit.serial_ports)
            {
                // If the port is connected, print this message
                if(entry.Value.port_connected == true)
                {
                    // Formats in the value of the port name, and the status of the port, into the localised string
                    Debug.Log(Localizer.Format(SERIAL_STATUS_MESSAGE, entry.Value.port_name, SERIAL_PORT_CONNECTED));
                }
                // If it is not connected, print this message
                else
                {
                    // Formats in the value of the port name, and the status of the port, into the localised string
                    Debug.Log(Localizer.Format(SERIAL_STATUS_MESSAGE, entry.Value.port_name, SERIAL_PORT_DISCONNECTED));
                }
            }
